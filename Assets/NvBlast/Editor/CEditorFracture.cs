...
public class CEditorTestFracture : EditorWindow {
    ...
    private void _createPreview(bool makePrefab) {
        ...
        for (int i = 1; i < fractureTool.getChunkCount(); i++) {
            ...
            ck.GetComponent<MeshCollider>().enabled = false;
            ck.GetComponent<MeshRenderer>().enabled = false;
            ck.GetComponent<Rigidbody>().isKinematic = true;
        }
        ...
        cs.transform.SetParent(source.transform, false);
        //cs.SetActive(false);

        Destructable destructable = source.AddComponent<Destructable>();
        Rigidbody rigidBody = source.AddComponent<Rigidbody>();

        destructable.rigidBody = rigidBody;
    ...
    private bool GUI_Voronoi() {
        ...
        totalChunks = EditorGUILayout.IntSlider("Total Chunks", totalChunks, 2, 1000);
